...
if ($object->xpdo) {
    ...
    switch ($options[xPDOTransport::PACKAGE_ACTION]) {
        ...
        case xPDOTransport::ACTION_UPGRADE:
            $modelPath = $modx->getOption('popularpages_core_path', null, $modx->getOption('core_path') . 'components/popularpages/') . 'model/';
            $modx->addPackage('popularpages', $modelPath);

            $manager = $modx->getManager();
	    
            $objects = array();
            $schemaFile = MODX_CORE_PATH . 'components/popularpages/model/schema/popularpages.mysql.schema.xml';
			...
			// Если таблица popularpages_setting
			// заполняем значениями по умолчанию
			if ($tmp == 'popularPagesSetting') {
				$row = $modx->newObject('popularPagesSetting');
				$row->fromArray(array(
					'id' => 1,
					'period' => '',
				));
